	...
	<div class="col-md-12">
		<div class="card">
  			<div class="card-header">
  				...
  				<div class="card-body">   
  			{!! Form::open(['url' => '/userPdf', 'target' => '_blank']) !!}
    		<p class="card-text">
				<div class="form-group row">    
    				<div class="col-md-12 form-inline justify-content-center">   
        				<select id="user_tipo" name="user_tipo" class="form-control{{ $errors->has('user_tipo') ? ' is-invalid' : '' }} col-sm-4">
            				<option value="" disabled selected>-- Seleccione Tipo Usuario  --</option>
            				<option value="T">Todos</option>           				
					            @foreach($users as $user)					                				                    
					                    <option value="{{ $user->tipo }}">@if($user->tipo == 1) Administrador 
					                    								  @elseif($user->tipo == 2) Dirección académica 
					                    								  @elseif($user->tipo == 3) Profesor
					                    								  @elseif($user->tipo == 4) Estudiante @endif
					                    </option>				                    
					            @endforeach
				        </select>    
						        @if ($errors->has('user_tipo'))
						            <span class="invalid-feedback" role="alert">
						                <strong>{{ $errors->first('user_tipo') }}</strong>
						            </span>
						        @endif
    				</div>
				</div>
